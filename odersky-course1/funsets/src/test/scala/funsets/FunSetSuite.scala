...
class FunSetSuite extends FunSuite {
  ...
  test("diff of two sets") {
    new TestSets {
      assert(!contains(diff(s2, s4), 2), "Diff 1")
  ...
  test("filter set") {
    new TestSets {
      assert(!contains(filter(s2, x => x > 2), 2), "Filter 1")
      assert(contains(filter(s3, x => x > 2), 3), "Filter 2")
    }
  }
  ...
  test("Forall") {
    new TestSets {
      val s = union(s1, s2)
      assert(forall(s, x => x > 0 && x < 3), "Forall 1")
